...
#'
#' @rdname greedySeqCA_naive
#'
#' @aliases greedySeqCA_naive
#' @aliases iter_greedySeqCA_naive
#'
#' @usage greedySeqCA_naive(k, t, seed=NULL, postopt=TRUE, ...)
#' @usage iter_greedySeqCA_naive(niter, k, t, seed_init=NULL, verbose=TRUE, ...)
#'
...
#'
#' @section Details:
#' Function \code{greedySeqCA_naive} starts from t! random permutations of the the
...
#'
#' Function \code{iter_greedySeqCA_naive} makes \code{niter} independent attempts and
...
#'
#' @seealso [greedySeqCA_Kuhn()] for the greedy method by Kuhn et al. (2012) and [greedySeqCA_TJ()] for the greedy stage method of Torres-Jimenez et al. (2022)
#'
...
#' Aiter <- iter_greedySeqCA_naive(10, 5, 3, seed_init=2415)
#' nrow(Aiter)
#' ## niter=20 better than niter=10
#' Aiter <- iter_greedySeqCA_naive(20, 5, 3, seed_init=2415)
#' nrow(Aiter)
#'
...
#' @export
greedySeqCA_naive <- function(k, t, seed=NULL, postopt=TRUE, ...){
...
#' @export
iter_greedySeqCA_naive <- function(niter, k, t, seed_init=NULL, verbose=TRUE, ...){
  ...
  for (i in 1:niter){
    ...
    hilf <- greedySeqCA_naive(k, t, postopt=FALSE, seed=seed)
