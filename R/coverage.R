...
coverage <- function(D, t, isInteger=TRUE,
  ...
  if (!is.logical(isInteger) || length(isInteger) != 1)
    stop("isInteger must be a single logical value (TRUE or FALSE)")
  if (!is.logical(start0) || length(start0) != 1)
    stop("start0 must be a single logical value (TRUE or FALSE)")
  ...
  if (!is.numeric(verbose) || length(verbose) != 1 || verbose < 0 || verbose != as.integer(verbose))
    stop("verbose must be a non-negative integer (0, 1, or 2)")
