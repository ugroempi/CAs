...
#' * the Sherwood, Martirosyan and Colbourn (2006) construction based on SCPHFs and permutation vectors, including not only the SCPHFs from the Sherwood et al. paper but also those provided by Walker and Colbourn (2009) and by Lanus (based on Colbourn and Lanus 2018 and Colbourn, Lanus and Sarkar 2018) in function \code{\link{scphfCA}}
...
#'
#' Furthermore, there are functions for the creation and inspection of sequence covering arrays
#' (SeqCAs) (Levenshtein 1991 as detailed in Na 2021, Kuhn et al. 2012, Murray and Colbourn 2015 (not yet implemented), Torres-Jimenez et al. 2022):
#'
#' * \code{\link{SeqCA_Levenshtein}} for algebraic creation of a SeqCA of strength t with t+1 columns,
#' * \code{\link{greedySeqCA_naive}}, \code{\link{greedySeqCA_Kuhn}}, \code{\link{greedySeqCA_TJ}}
#' and their iterative versions,
#' * \code{\link{coverageSeqCA}} and \code{\link{identify_needed}} for assessing coverage and usefulness / redundancy of SeqCA entries,
#' * \code{\link{reduce_rows_iterative_complete}} and \code{\link{optimize_SeqCA}} for reducing the run size of a SeqCA.
#'
#' These functions are limited to SeqCAs for relatively small numbers of columns and strengths.
#'
...
#' * \code{\link{PCAcat}}
#' * \code{\link{DPcat}}
#' * \code{\link{SCPHFcat}}, \code{\link{SMC_SCPHFs}}, \code{\link{CL_SCPHFs}}, \code{\link{WC_SCPHFs}}
...
#'
#' Kuhn, D.R., Higdon, J.M., Lawrence, J.F., Kacker, R.N. (2012). Combinatorial Methods for Event Sequence Testing. ICST '12: Proceedings of the 2012 IEEE Fifth International Conference on Software Testing, Verification and Validation, 601-609. https://doi.org/10.1109/ICST.2012.147
#'
#' Levenshtein, V.I. (1991). Perfect codes in the metric of deletions and insertions. Diskretnaya Matematik 3(1), 3-20.
#'
...
#'
#' Na, J. (2021). Perfect Sequence Covering Arrays. Master's thesis at Simon Fraser University. https://summit.sfu.ca/_flysystem/fedora/2022-09/etd21570.pdf
#'
...
#'
#' Torres-Jimenez, J. (without year, accessed 10 Feb 2025). Covering arrays. Formerly available at \code{https://www.tamps.cinvestav.mx/~oc/} (no longer functional, no alternative location known).
#'
...
#'
#' Torres-Jimenez, J., Ramirez-Acuna, D.O., Acevedo-Juarez, B., Avila-George, H. (2022). New upper bounds for sequence Covering Arrays using a 3-stage approach. Expert Systems With Applications 207, 118022. https://doi.org/10.1016/j.eswa.2022.118022
#'
...
#'
#' Walker II, R. and Colbourn, C. (2009). Tabu search for covering arrays using permutation vectors. Journal of Statistical Planning and Inference 139, 69-80.
#'
...
#' @importFrom utils globalVariables combn modifyList
