...
#'
#' @rdname recursiveBoseHartman
#'
#' @aliases recursiveBoseHartman
#' @aliases N_k_recursiveBoseHartman_A
#' @aliases N_k_recursiveBoseHartman_d
#' @aliases N_d_recursiveBoseHartman
#'
#' @usage recursiveBoseHartman(q, A=NULL, d=NULL, ...)
#' @usage N_k_recursiveBoseHartman_A(q, A, ...)
#' @usage N_k_recursiveBoseHartman_d(q, d=2, ...)
#' @usage N_d_recursiveBoseHartman(q, k, ...)
#'
...
#'
#' @returns \code{recursiveBoseHartman} returns a uniform strength 2 CA with \code{q} level columns.\cr
...
#' For \code{A=NULL}, the function uses `lhs::createBose(q, q+1, bRandom=FALSE)` for creating A
...
#'
#' @export
recursiveBoseHartman <- function(q, A=NULL, d=NULL, ...){
  start0 <- TRUE ## default
  if (!is.null(A)) d <- NULL
  if (!is.null(d)){
...
#' @export
N_k_recursiveBoseHartman_A <- function(q, A, ...){
...
#' @export
N_k_recursiveBoseHartman_d <- function(q, d=2, ...){
  ## check for d
  stopifnot(is.numeric(d))
  stopifnot(d>0)
  stopifnot(d %% 1 == 0)
...
#' @export
N_d_recursiveBoseHartman <- function(q, k, ...){
  ## check for k
