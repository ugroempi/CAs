...
#' @aliases derivefrom
#'
#' @usage derive(D, dcol=1, dval=NULL, ...)
#' @usage deriveto(N, k, v, ...)
#' @usage derivefrom(N, k, v, ...)
#'
#' @param D a CA with levels as integers 0, ..., v-1 or 1,...,v
#' @param dcol the id of the column to be deleted (default: 1)
#' @param dval the value to be used for filtering (default: the first value that has minimum frequency in column \code{dcol})
#' @param N integer: run size;\cr
...
#' @param k integer: number of columns;\cr
...
#' @param v integer: the number of levels (same for ingoing and resulting array)
...
#'
#' @returns \code{derive} returns a matrix with class \code{ca}, which has strength one
...
#'
#' @export
derive <- function(D, dcol=1, dval=NULL, ...){
  hilf <- matcheck(D, uniform=FALSE)
  k <- hilf$k
  vs <- hilf$vs
  start0 <- hilf$start0
  if (!dcol %in% 1:k) stop("dcol must be a column number for D")
  if (is.null(dval)){
    tab <- table(D[,dcol])
    dval <- as.numeric(names(tab)[which.min(tab)])
  }
  if (!(dval %in% ((1:vs[dcol]) - as.numeric(start0)))) stop("invalid dval")
...
#' @export
deriveto <- function(N, k, v, ...){
  stopifnot(is.numeric(N), is.numeric(k), is.numeric(v))
  stopifnot(N%%1==0, k%%1==0, v%%1==0)
  c(N = floor(N/v), k=k-1)
}

#' @export
derivefrom <- function(N, k, v, ...){
  stopifnot(is.numeric(N), is.numeric(k), is.numeric(v))
  stopifnot(N%%1==0, k%%1==0, v%%1==0)
  list(N=(N*v):((N+1)*v-1), k=k+1)
}
