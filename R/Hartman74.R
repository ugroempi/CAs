...
#' @usage Turan(t, v)
#'
#' @param A an N x k CA of strength \code{t} columns in \code{v} levels each, denoted as 0 to v-1 or 1 to v
...
#'
#' @returns function \code{Hartman74} returns a strength \code{t} CA with \code{k^2} columns,\cr
...
Hartman74 <- function(A, v, t, B=NULL, ...){
  ...
  if (ncol(B) < colBneeded) {
    ...
    message("you can try to use an A with more columns, which will imply more columns in B")
    stop("too few columns in B,\n at least ", colBneeded, " columns needed")
  }
  B <- maxconstant(B, remove=TRUE)
