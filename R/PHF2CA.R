...
#' @param D an N x w CA of strength t with v levels
...
PHF2CA <- function(P, D, ...){
  ...
  const <- as.list(Drem[1:nconst,1])
  Drem <- Drem[-(1:nconst),]
  aus <- matrix(NA, (N-nconst)*M, k)
  for (i in 1:M){
      aus[(i-1)*(N-nconst) + 1:(N-nconst),] <- Drem[,P[i,]]
  }
  do.call(rbind, c(const, list(aus)))
}
