...
## coverage OK
SMC_SCPHFs$`3`[["4"]] <- list("16"=cphf1, "28"=cphf2)
...
(cphf <- transform(cphf, 3))
SMC_SCPHFs$`4`[["5"]] <- list("11"=cphf)

SCPHFcat <- data.frame(t=numeric(0), v=numeric(0), k=numeric(0), N=numeric(0))
for (t in 3:4)
  for (v in as.numeric(names(SMC_SCPHFs[[as.character(t)]]))){
    for (k in as.numeric(names(SMC_SCPHFs[[as.character(t)]][[as.character(v)]]))){
      kc <- as.character(k); vc <- as.character(v); tc <- as.character(t)
      SCPHFcat <- rbind(SCPHFcat,
      c(t=t, v=v, k=k,
        N=nrow(SMC(SMC_SCPHFs[[tc]][[vc]][[kc]], v, t))))
    }
  }
...
save("SMC_SCPHFs", file="d:/rtests/CAs/data/SMC_SCPHFs.rda", compress="xz")
